...
public class HashBasedRouter extends BaseRouter {
	...
	public Collection<Shard> getSearchShardsSingle(String shardKey,
		...
		if (m1 == 0) {
			...
			// 0x000000000->0xffffffff which only works on unsigned space, but
			// we're using signed space.
	...
	public List<Range> partitionRange(int partitions, Range range) {
		...
		// keep track of the idealized target to avoid accumulating rounding
		// errors
		long targetStart = min;
		long targetEnd = targetStart;

		// Round to avoid splitting hash domains across ranges if such rounding
		// is not significant.
		// With default bits==16, one would need to create more than 4000 shards
		// before this

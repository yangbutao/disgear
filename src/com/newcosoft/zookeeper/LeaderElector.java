...
public class LeaderElector {
	 ...
	 * watching the candidate that is in line before this one - if it goes down,
	 * check if this candidate is the leader again.
	 * 
	 * @param replacement
	 *            has someone else been the leader already?
	 */
	private void checkIfIamLeader(final int seq, final BaseElectionContext context,
		...
		if (seq <= intSeqs.get(0)) {
			// first we delete the node advertising the old leader in case the
			// ephem is still there
			try {
	 ...
	 * and begins watching the node with the sequence number before it, unless
	 * it is the lowest number, in which case, initiates the leader process. If
	 * the node that is watched goes down, check if we are the new lowest node,
	 * else watch the next lowest numbered node.
	 * 
	...
	public String joinElection(BaseElectionContext context, boolean replacement)
		...
		while (cont) {
			try {
				...
				// we must have failed in creating the election node - someone
				// else must
